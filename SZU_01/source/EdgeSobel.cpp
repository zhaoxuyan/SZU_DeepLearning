...
void CEdgeSobel::MySobel(Mat InputImg, Mat &Gray32Mat) {
//  InputImg 是灰度图像
    ...
    for (int j = 0; j < InputImg.rows; ++j) {
        for (int i = 0; i < InputImg.cols; ++i) {
            ...
            for (int p = -oh; p <= oh; ++p) {
                for (int q = -ow; q <= ow; ++q) {
                    if (p + j < 0 || p + j > InputImg.rows - 1 ||
                        q + i < 0 || q + i > InputImg.cols - 1) {
                        continue;
                    }
                    datavalue += dataMat(j + q, i + q) * m_OperatorX[(p + oh) * OperatorSize + (q + ow)];
                    datavalue += dataMat(j + q, i + q) * m_OperatorY[(p + oh) * OperatorSize + (q + ow)];
                }
            }
